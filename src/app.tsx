...
export function App() {
  const [data, dispatch] = useStorageReducer<Planner | DraftPlanner, Action>(
    (state, action) => {
      switch (action.type) {
  ...
  const totalAmount =
    availableToSpent > totalSpending ? availableToSpent : totalSpending;
  ...
  function getDataFromForm(form: HTMLFormElement): Spent {
    ...
    if (amountString.length === 1) amountString = '0' + amountString;
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <header className="h-16 bg-primary w-full px-4 flex justify-between items-center">
        <Input
          key={'id' in data ? data.id : 'default-key'}
          placeholder="Planner Title"
          className="max-w-96"
          value={data.title}
          onChange={v =>
            dispatch({ type: Actions.UPDATE_TITLE, payload: v.target.value })
          }
        />
        ...
        <div className="flex items-center gap-1">
          <DropdownMenu.Provider>
            <DropdownMenu.Trigger asChild disabled={!planners.length}>
              <Button
                onClick={clearBoard}
                disabled={!planners.length}
                className="gap-2 bg-primary text-white hover:bg-white hover:text-primary rounded"
              >
      ...
      <div className="h-full p-4 flex gap-4 w-full">
        <div className="flex flex-col h-full justify-between border border-border rounded bg-card max-w-xl flex-shrink-0 basis-[576px] overflow-hidden">
              ...
              return (
                <li key={spend.id}>
                  <form
                    ...
                    onSubmit={onEditSubmit(spend.id!)}
                  >
                    <div className="flex items-center gap-2">
                      <PaletteInput
                        options={COLOR_OPTIONS}
                        defaultValue={spend.color}
                      />
          ...
          <form
            key={tick}
            className="flex items-center gap-2 w-full border-t border-border  p-4"
        ...
        <div className="flex-1 border border-border p-4 rounded flex flex-col justify-between">
          <div className="w-full flex flex-wrap gap-4 justify-between">
            <p className="flex items-center gap-4">
              ...
              <Input
                readOnly
                value={formatToCurrency(totalSpending)}
            ...
            {isAvailableDefined && (
              <p className="flex items-center gap-4">
                ...
                <Input
                  readOnly
                  value={formatToCurrency(Math.abs(rest))}
                  className={dangerClasses}
                />
              </p>
            )}
